...
def run(playwright: Playwright) -> None:
    ...
    file_name_selector = "#__next > div.grid.grid-cols-only-content.lg\\:grid-cols-sidebar-content.\\32 xl\\:grid-cols-sidebar-content-toc > div.lg\\:-mt-16.z-10 > div > div > div > aside > nav > ul > h3:nth-child(1)"
    ...
    for link in links:
        ...
        # Check if 'react-dom' is in the href attribute
        if 'react-dom' in href:
            break  # Exit the loop if 'react-dom' is found
...
add_docfile("React", file_name_text.split("@")[1], "L", file_name, file_bytes)
